...
- 新增 WebhookCardTest(卡片中文化断言,2 用例)

### Fixed（个人主页装修模式重做 · 用户反馈）
- **装修编辑器从弹窗改为页内「装修模式」**:原弹窗(672px)比所编辑的页面内容(896px)还窄、且 `lg` 档无 max-height——内容一高(自定义 CSS 页)整个弹窗超出视口又不可滚动,底部保存按钮被顶出屏幕(用户报「无保存按钮/空太小/上下不对齐」)。现点击「自定义主页」进入装修模式:顶部**吸顶操作栏**(退出装修/保存修改常驻可见),预览下方**同宽对齐**的「装修配置」面板(简介/横幅与背景/社交链接/自定义 CSS),改动即时预览;`?customize=1` 深链直达装修模式;入口按钮图标去 emoji 改 AppIcon
- **表单重排**:横幅图补上传按钮(原来只有背景图能传);简介字数计数并入标签;主题色色板加大(64×40);社交链接行删除按钮规格化;各输入框放开到内容区全宽
- **修「取消不还原」**:编辑中实时预览会直接覆写已保存数据,原实现退出后旧值回不来;现进入装修模式时留存快照,退出未保存即还原(含自定义 CSS 注入);离开页面时移除注入的 `#pc-style`,未保存 CSS 不再泄漏到其它页面
- **全站 sticky 失效修复**:`body{overflow-x:hidden}` 会把 body 变成滚动容器,子元素 `position:sticky` 全部失效(个人主页吸顶栏、文档页侧栏同病);body 改用 `overflow-x:clip`(不产生滚动容器),吸顶恢复
- **移除右上角「主页设置」入口**:用户菜单与移动端菜单原「主页设置」(/player/{u}?customize=1)删除——个人主页已有「自定义主页」按钮进入装修模式,入口冗余(用户要求,已确认);`?customize=1` 深链解析保留
- 顺手:`saveProfileCustom` 参数类型补齐 bgImage/bannerImage/accent/css;清理未用变量与死代码

### Fixed（玩家详情页空白 + 称号入口调整 · 用户反馈）
- **玩家详情页全空白(根因)**:M3 加「个人主页自定义」时,PlayerProfile 使用了未导入的 `computed`/`inject`——setup 阶段 ReferenceError 导致所有玩家详情页渲染即白屏;已补全 vue 导入与 notify 注入
- **「我的称号」合并进控制台**:称号面板(佩戴/脱下/成就进度)从玩家资料页迁入控制台 Dashboard
- **个人主页入口**:顶栏用户菜单原「我的称号」改为「个人主页」(桌面+移动),直达公开资料页(不再依赖 ?titles=1 深链)

### Added（六项功能批次：评分看板 · 主页自定义 · 好友 · 活动日历 · 论坛图片/富文本）
- **玩家综合评分 + 行为分析看板**:五项指标封顶归一加权(权重后台可调,publicScore 可关);排行榜新增「综合评分」榜、PlayerProfile 评分卡;Admin 行为分析区(DAU/注册趋势/聊天量/Top 活跃玩家)
- **玩家个人主页自定义**:V16 dp_user_profile;简介(≤500)+横幅预设色板(6 色)+社交链接(≤5 条,仅 http/https);公开资料页展示,本人资料页编辑
- **好友系统**:V17 dp_friendship(双向去重);申请/同意/拒绝/删除;资料页加好友按钮状态感知;控制台好友卡(在线标记用心跳名单比对)
- **活动日历**:V18 dp_event+dp_event_signup;管理端 CRUD+报名名单;社区页「日历」tab(月历网格+报名/取消+人数)
- **论坛图片上传 + 富文本编辑器**:通用上传端点 POST /api/upload/image(JWT/5MB/magic bytes);RichEditor 组件(Tiptap + tiptap-markdown,markdown 进出零迁移);工具栏 H1-H3/粗斜删/颜色/列表/引用/代码/链接/图片上传/清除;接入 Admin 文档管理与论坛发帖/回帖
- **通知后端删除**:V15 DROP dp_notification;notification 包与 15 处写入点、4 个端点移除;邮件通道全保留
- 鉴权重构:InternalController 20 处 server-token 鉴权抽为 security/ServerTokenVerifier 组件共用
- 冒烟全通过:主页自定义保存/带出、好友申请双向隔离、活动报名人数、图片上传(成功/401)、评分榜/本人评分/行为分析

### Fixed（公告/社区页头玻璃与背景冲突 · 用户反馈）
- 公告页与社区页的页头玻璃卡与背景图叠加显得浑浊(用户此前已有「标题后不加玻璃面板」的口味记录)——页头玻璃面板移除,节头+分类 tab 直接落在背景上,下方内容卡的玻璃保留
...
- 特色区补齐漏掉的改版:bento 不等宽网格(首卡跨两列)+图标左置小方框+hover 转橙
  (此前仅旧版样式,与其他区块不一致)
...
### Changed（门户首屏与照片墙 · 用户反馈）
- **修复首页刷新闪烁**:揭幕时机改为等「最终背景图真实解码完成」(自定义背景 webp→原图探针链 Promise 化,5s 兜底)——此前只等默认 bg.webp,自定义背景在揭幕后才突然出现;配置加 localStorage SWR 缓存,刷新时用上次配置同步渲染(背景/品牌色/Logo/轮播/照片墙秒出),后台静默拉最新覆盖;Portal 组件同理种子化,内容不再蹦出
- **时光照片墙改版「杂志瀑布流」**(呈现层重做,分类筛选/点击放大留言/后台配置不变):CSS columns 瀑布流保留原图比例(不再强制 4:3);衬线斜体「Moments」刊头水印+照片右下大号年份水印;分类筛选改杂志 kicker 下划线指示器;hover 图片微缩放+标题动效下划线+橙色 ring/深阴影;去卡片底框改纯图注排版

### Fixed（玩家列表/详情页头像显示默认脸 · 用户反馈）
- **根因**:玩家总览(Players.vue)与详情页头部(PlayerProfile.vue)的头像 URL 传的是**账号名**(`player.username`),而头像服务按**游戏名**解析——改过 ID 的玩家账号名≠游戏名,总览必然默认脸;叠加列表页 20+ 头像并发冷缓存更易撞上 Mojang 瞬时故障(服务端上一条修复前还会把默认脸缓存 1h)
- **修复**:两处统一为「绑定 MC 名 → 账号名」解析口径(与控制台头像一致),优先用接口返回的 `minecraftName`

### Fixed（头像偶发显示默认脸 · 用户反馈）
- **根因**:冷缓存(首次遇到/内存过期)时,Mojang 查档或皮肤下载的**瞬时故障**(超时/429/5xx,国内网络常见)降级的默认脸被当正式结果**写入内存缓存 1 小时**,且默认脸响应带 `max-age=3600` 把浏览器也钉住 1 小时——一次抖动=一个小时内该头像持续默认
- **修复**:①瞬时失败与终态区分——查档/皮肤获取的瞬时异常与 429/5xx 抛出,本次降级默认脸但**不写任何缓存**(30s 重试节流防故障期打爆 Mojang),下次请求自动重试;官方库无此名(204/404)与无材质仍是终态、正常缓存;②默认脸响应 Cache-Control 降为 `max-age=60`,浏览器最快 1 分钟内跟随服务端恢复;③官方皮肤一旦成功落盘仍"永不等网络"(SWR 静默刷新),稳定不变
- 新增 AvatarTransientTest 3 例(瞬时失败不缓存/重试节流/终态正常缓存),共 42 测试全绿

## [1.5.0] - 2026-09-10

**三阶段功能补强发版（差距评估驱动）：监控与多服 · 社区与内容 · 两步验证。**
...
### Fixed（发布前代码审计）
- SimpleRateLimiter 清理定时化(每小时,此前 cleanup 无调用点,日桶/窗口键随运行时间无限增长)
- 2FA 管理员强制门收紧:绑定中(pending)不再豁免 needs_2fa_setup,防"开始绑定即绕过强制"的半途状态
- 反馈建单/投票创建/论坛发帖的"SELECT id 回查"改为 GeneratedKeyHolder,消除同用户同毫秒并发下 ID 错位竞态
- 移除 Admin.vue 未使用的 loadSecurity 死代码
