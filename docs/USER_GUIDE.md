...
| 主页装修 | 个人主页 → 自定义主页 | 简介/横幅图/主题色/背景图/社交链接/自定义 CSS；装修模式所见即所得实时预览，保存后对外生效 |
...
| 邮件群发 | 向所有注册并绑定邮箱的玩家发邮件（主题≤200/正文≤20000，发送前确认，异步投递；未配 SMTP 走日志模式） |
| SEO 搜索优化 | 门户管理 → SEO 搜索优化：标题模板/描述/关键词/分享图/收录开关/自定义 head 注入；自动生成 /robots.txt 与 /sitemap.xml；服务端直出 meta，爬虫免渲染 |
...
- **主页装修**：个人主页点「自定义主页」进入装修模式——顶部吸顶操作栏（退出装修/保存修改），预览下方同宽「装修配置」面板；简介/横幅图/主题色/背景图/社交链接/自定义 CSS 实时预览；自定义 CSS 服务端消毒 + `#pc-root` 作用域限定；退出未保存自动还原
- **邮件群发**（后台「邮件群发」）：向所有绑定邮箱玩家群发，同一邮箱只发一封；发送前二次确认
- **Webhook 飞书卡片中文化**：事件中文名 + 字段中文标签 + 格式化时间
- **终端访问日志中文输出**：常用接口中文描述（如「玩家资料」「插件轮询·待处理消息」），未知接口动词中文化
